...
fun groupDistributedIntakesV1(
    notes: List<Annotation>,
    gapMin: Long = DISTRIBUTED_INTAKE_GAP_MIN_V1,
    maxSpanMin: Long = DISTRIBUTED_INTAKE_MAX_SPAN_MIN_V1
): List<DistributedIntakeV1> {
    ...
    notes
        ...
        .forEach { n ->
            ...
            if (
                cur != null &&
                    sameRescue &&
                    n.tsMs - cur.last().tsMs <= allowed * 60_000 &&
                    n.tsMs - cur.first().tsMs <= maxSpanMin * 60_000
            )
                cur.add(n)
    ...
    return groups.map {
        DistributedIntakeV1(
            "meal:${it.first().tsMs}:${it.joinToString(","){n->n.id.toString()}}",
            it
        )
    }
}

data class TimeContributionPointV2(
    val tsMs: Long,
    val median: Double,
    val low: Double,
    val high: Double
...
data class TimeFitBinV2(
    val tsMs: Long,
    val observedAfterInsulin: Double,
    val fittedMeals: Double,
    val fittedBackground: Double,
    val sharedResidual: Double,
    val fittedTotal: Double,
    val sigma: Double,
)
...
data class ClusterTimingV1(
    val onsetMin: Double,
    val levelPeakMin: Double,
    val mainEndMin: Double,
...
data class MealAllocationV1(
    ...
    val startMs: Long,
    val endMs: Long,
    val recordedCarbsG: Double,
    val gramsProvenance: String,
    val gramsLow: Double,
    val gramsHigh: Double,
    val modelForecastMmol: Double,
    val bestAllocationMmol: Double,
    val allocationLowMmol: Double,
    val allocationHighMmol: Double,
    val shareMedian: Double,
    val shareLow: Double,
    val shareHigh: Double,
    val status: AttributionResolutionV1,
    val observedPhases: Set<ObservedPhaseV1>,
    val phaseWindowsAvailable: Set<ObservedPhaseV1>,
    val coverageMin: Double,
    val perNoteOnsetsMs: List<Long>,
    val confidence: Double,
    val caveats: List<String>,
    val contributionCurve: List<TimeContributionPointV2>,
    val fractionBeforeNext: PosteriorV1,
    val fractionAfterNext: PosteriorV1,
    val priorStrength: Double,
    val causalOnsetMs: Long,
...
data class JointAttributionReceiptV1(
    ...
    val allocations: List<MealAllocationV1>,
    val unloggedFoodResidualMmol: PosteriorV1,
    val backgroundMmolPerHour: PosteriorV1,
    val insulinKernelHashes: Map<Long, String>,
    val clusterObservedAfterInsulinBackground: PosteriorV1,
    val status: AttributionResolutionV1,
    val cacheIdentity: String,
    val timeBins: List<TimeFitBinV2>,
    val sharedResidualCurve: List<TimeContributionPointV2>,
    val tailTransferAudit: TailTransferAuditV2?,
...
data class JointAttributionConfigV1(
    val horizonMin: Double = 360.0,
    val gridMin: Double = 15.0,
    val mealAmplitudeMaxPrior: Double = 1.60,
    val backgroundAbsMaxMmolPerHour: Double = 1.5,
    val anchorPenalty: Double = 5.0,
    val residualPenalty: Double = 12.0,
    val residualSmoothness: Double = 5.0,
    val sensorLagMedianMin: Double = 10.0,
    val sensorLagLowMin: Double = 5.0,
    val sensorLagHighMin: Double = 20.0,
    val sensorSigmaMmol: Double = .18,
    val maxIterations: Int = 140,
    val maxNotes: Int = 24,
    val maxCgmGapMin: Double = 30.0,
    val minCoverageDensity: Double = .60,
...
private fun shapeSpec(n: Annotation, config: JointAttributionConfigV1, asOfMs: Long): ShapeSpec {
    ...
    val fat = (nutrition.fatG ?: 0.0);
    val protein = (nutrition.proteinG ?: 0.0)
    ...
    return ShapeSpec(
        listOf(
                ShapePartV2(
                    k.fastFraction,
                    25.0 * formStretch,
                    100.0 * formStretch + macroTail * .45
                ),
                ShapePartV2(
                    k.mediumFraction,
                    55.0 * formStretch,
                    220.0 * formStretch + macroTail * .75
                ),
                ShapePartV2(k.slowFraction, 90.0 * formStretch, 360.0 * formStretch + macroTail),
            )
            .filter { it.weight > 1e-6 },
        config.sensorLagMedianMin,
        protein,
        fat,
        fiber,
        false
...
private fun shapeProgressAtAge(ageMin: Double, spec: ShapeSpec, lag: Double = spec.lag): Double {
    ...
    if (tau <= 0) return 0.0
    return spec.parts
        .sumOf { it.weight * (1 - exp(-tau / it.rise.coerceAtLeast(1.0))) }
        .coerceIn(0.0, 1.0)
}
private fun clusterMember(n: Annotation, spec: ShapeSpec): MealClusterMemberV1 {
    ...
    val tail =
        (spec.parts.maxOfOrNull { it.tail * 2.0 } ?: 360.0).coerceIn(main + 1, 720.0) + spec.lag
    return MealClusterMemberV1(
        "food:${n.id}",
        n.tsMs,
        spec.proteinG,
        spec.fatG,
        spec.fiberG,
        main,
        tail.coerceAtMost(720.0)
    )
}
private fun provenancePrior(source: String?): Pair<Double, Double> =
...
fun jointMealAttributionV1(
    ...
    val eligibleAll =
        notes
            .filter { it.carbsKnownAtMs?.let { known -> known <= asOfMs } == true }
            .sortedBy { it.tsMs }
    ...
    val eligible =
        if (inputTruncated && config.maxNotes > 1)
            eligibleAll.take(config.maxNotes - 1) + eligibleAll.last()
        else eligibleAll.take(config.maxNotes)
    val sessions =
        groupDistributedIntakesV1(eligible).filter { it.carbsG > 0 && it.startMs <= asOfMs }
    if (sessions.isEmpty() || readings.isEmpty()) return null
    val start = sessions.minOf { it.startMs };
    val intendedEnd = minOf(asOfMs, start + (config.horizonMin * 60_000).toLong())
    val r = readings.filter { it.tsMs <= asOfMs }.sortedBy { it.tsMs }
    fun near(t: Long) =
        r.minByOrNull { abs(it.tsMs - t) }?.takeIf { abs(it.tsMs - t) <= 10 * 60_000 }
    // Never anchor on post-meal glucose: even +10 minutes can already contain food.
    val basePoint =
        r.filter { it.tsMs <= start }
            .maxByOrNull { it.tsMs }
            ?.takeIf { start - it.tsMs <= 15 * 60_000 } ?: return null
    val base = basePoint.mmol
    val times =
        generateSequence(start + (config.gridMin * 60_000).toLong()) {
                it + (config.gridMin * 60_000).toLong()
            }
            .takeWhile { it <= intendedEnd }
            .mapNotNull { t -> near(t)?.let { p -> Triple(t, p.tsMs, p.mmol) } }
            .distinctBy { it.second }
            .toList()
    if (times.size < 3) return null
    fun continuousEnd(onset: Long): Long {
        val points =
            (listOfNotNull(r.filter { it.tsMs <= onset }.maxByOrNull { it.tsMs }) +
                    r.filter { it.tsMs > onset && it.tsMs <= intendedEnd })
                .sortedBy { it.tsMs }
        if (points.isEmpty()) return onset
        var end = points.first().tsMs
        for (p in points.drop(1)) {
            if ((p.tsMs - end) / 60_000.0 > config.maxCgmGapMin) break;
            end = p.tsMs
        }
        return end
    }
    ...
    fun insulin(t: Long, which: Int) = activeBoluses.sumOf { b ->
        val k = kernels.getValue(b).points
        b.units *
            (gv(k, (t - b.tsMs) / 60_000.0, which) - gv(k, (start - b.tsMs) / 60_000.0, which))
    }
    val y = DoubleArray(times.size) { i -> times[i].third - base - insulin(times[i].first, 0) }
    val sigma =
        DoubleArray(times.size) { i ->
            val lo = insulin(times[i].first, -1);
            val hi = insulin(times[i].first, 1)
            sqrt(
                    config.sensorSigmaMmol * config.sensorSigmaMmol +
                        ((hi - lo) / 2.5632).let { it * it }
                )
                .coerceAtLeast(.12)
        }
    ...
    val clusterMembers =
        specs.flatMap { it.entries }.associate { (n, spec) -> n to clusterMember(n, spec) }
    ...
    val appearanceMembers = normalNotes.map { n ->
        ...
        val nutrition =
            com.diapilot.core.analysis.parseFoodNutrition(
                n.analysis.takeIf { n.analysisKnownAtMs?.let { known -> known <= asOfMs } == true },
            )
    ...
    fun rawClusterProgress(n: Annotation, t: Long, spec: ShapeSpec): Double {
        val clock =
            causalMealClusterClockV1(
                clusterMembers.getValue(n),
                t,
                normalClusterMembers,
        ...
        limitedProgressCache.putAll(
            appearance.emptyingKcalPerHour?.let { rate ->
                caloricLimitedTimelineV1(
                    ...
                    supply,
                    kcalPerHour = rate,
                    carbSieving = appearance.carbSieving
                )
            }
    ...
    fun limitedProgress(n: Annotation, t: Long, spec: ShapeSpec): Double {
        ...
        val map =
            limitedProgressCache.getOrPut(t) {
                val supply = { m: CarbAppearanceMemberV1, at: Long ->
                    val note = noteByAppearanceId.getValue(m.id.removePrefix("food:"))
                    rawClusterProgress(note, at, specByNote.getValue(note))
                }
    ...
    fun sessionBasis(j: Int, t: Long, lagMode: Int = 0): Double {
        val known = sessions[j].notes.filter { (it.estCarbs ?: 0.0) > 0 };
        val total = known.sumOf { it.estCarbs!! }.coerceAtLeast(1e-9)
        return known.sumOf { n ->
            val sp = specs[j].getValue(n);
            val lag =
                when (lagMode) {
                    -1 -> config.sensorLagLowMin;
                    1 -> config.sensorLagHighMin;
                    else -> sp.lag
                }
    ...
    fun sessionProgress(j: Int, t: Long): Double {
        ...
        return known
            .sumOf { n ->
                ...
                itWeight(n, total) *
                    (if (sp.rescue) shapeProgressAtAge(clock.effectiveAgeMin, sp, sp.lag)
                    else limitedProgress(n, t, sp))
            }
            .coerceIn(0.0, 1.0)
    }
    val x =
        sessions.indices.map { j ->
            DoubleArray(times.size) { i ->
                if (times[i].second <= continuousEnds[j]) sessionBasis(j, times[i].first) else 0.0
            }
        }
    val xEarly =
        sessions.indices.map { j ->
            DoubleArray(times.size) { i ->
                if (times[i].second <= continuousEnds[j]) sessionBasis(j, times[i].first, -1)
                else 0.0
            }
        }
    val xLate =
    ...
    val priorStrength =
        DoubleArray(sessions.size) { j ->
            val total = sessions[j].carbsG.coerceAtLeast(1e-9);
            sessions[j].notes.sumOf {
                (it.estCarbs ?: 0.0) / total * provenancePrior(it.carbsSource).first
            }
        }
    val gramUncertainty =
        DoubleArray(sessions.size) { j ->
            ...
            sqrt(
                sessions[j].notes.sumOf {
                    val w = (it.estCarbs ?: 0.0) / total;
                    val u = provenancePrior(it.carbsSource).second;
                    w * w * u * u
                }
            )
        }
    val beta = prior.copyOf();
    val process = DoubleArray(times.size);
    var bg = 0.0
    ...
    repeat(config.maxIterations) {
        ...
        val previousBeta = beta.copyOf();
        val previousProcess = process.copyOf();
        val previousBg = bg
        fun previousPred(i: Int) =
            x.indices.sumOf { j -> x[j][i] * previousBeta[j] } +
                bgBasis[i] * previousBg +
                previousProcess[i]
        val nextBeta = beta.copyOf()
        for (j in x.indices) {
            var num = priorStrength[j] * prior[j];
            var den = priorStrength[j]
            for (i in y.indices) {
                val w = 1 / (sigma[i] * sigma[i]);
                val without = previousPred(i) - x[j][i] * previousBeta[j];
                num += w * x[j][i] * (y[i] - without);
                den += w * x[j][i] * x[j][i]
            }
        ...
        val priorTotal = prior.sum().coerceAtLeast(1e-9);
        val fittedTotal = beta.sum()
        val strong = priorStrength.average() >= 10.0;
        val lo = priorTotal * (if (strong) .70 else .50);
        val hi = priorTotal * (if (strong) 1.30 else 1.60)
        if (fittedTotal > 1e-9 && fittedTotal !in lo..hi) {
            val scale = (fittedTotal.coerceIn(lo, hi) / fittedTotal);
            for (j in beta.indices) beta[j] *= scale
        }
        run {
            var num = 0.0;
            var den = config.anchorPenalty * 2.0;
            for (i in y.indices) {
                val w = 1 / (sigma[i] * sigma[i]);
                val without = pred(i) - bgBasis[i] * bg;
                num += w * bgBasis[i] * (y[i] - without);
                den += w * bgBasis[i] * bgBasis[i]
            };
            bg =
                (num / den).coerceIn(
                    -config.backgroundAbsMaxMmolPerHour,
                    config.backgroundAbsMaxMmolPerHour
                )
        }
        for (i in process.indices) {
            val meals = x.indices.sumOf { j -> x[j][i] * beta[j] };
            val raw =
                (y[i] - meals - bgBasis[i] * bg) /
                    (1 + config.residualPenalty * sigma[i] * sigma[i])
            val neighbour =
                ((process.getOrNull(i - 1) ?: 0.0) + (process.getOrNull(i + 1) ?: 0.0)) / 2
            process[i] =
                ((raw + config.residualSmoothness * neighbour) / (1 + config.residualSmoothness))
                    .coerceIn(-12.0, 12.0)
        }
    }
    val errors = DoubleArray(y.size) { y[it] - pred(it) };
    ...
    val signedProcess =
        DoubleArray(process.size) { i -> (process[i] + errors[i]).coerceIn(-12.0, 12.0) }
    fun jointCosine(a: DoubleArray, b: DoubleArray): Double {
        val support = a.indices.filter { a[it] > .03 && b[it] > .03 };
        if (support.size < 3) return 0.0
        val ab = support.sumOf { a[it] * b[it] / (sigma[it] * sigma[it]) };
        val aa = sqrt(support.sumOf { a[it] * a[it] / (sigma[it] * sigma[it]) });
        val bb = sqrt(support.sumOf { b[it] * b[it] / (sigma[it] * sigma[it]) })
        return if (aa * bb < 1e-9) 0.0 else ab / (aa * bb)
    }
    val total = (beta.sum() + process.maxOrNull().orZero()).coerceAtLeast(1e-9)
    val intervalLow = DoubleArray(beta.size);
    val intervalHigh = DoubleArray(beta.size)
    val allocations =
        sessions.indices.map { j ->
            val previousClose =
                j > 0 && (sessions[j].startMs - sessions[j - 1].endMs) / 60_000.0 <= 120.0
            val nextClose =
                j < sessions.lastIndex &&
                    (sessions[j + 1].startMs - sessions[j].endMs) / 60_000.0 <= 120.0
            val previousStillActive =
                previousClose && sessionProgress(j - 1, sessions[j].startMs) < .90
            val completedBeforeNext =
                nextClose && sessionProgress(j, sessions[j + 1].startMs) >= .90
            val timingScope =
                when {
                    completedBeforeNext -> MealTimingScopeV1.COMPLETED_BEFORE_CLUSTER
                    previousStillActive || nextClose -> MealTimingScopeV1.CLUSTER_ONLY
                    else -> MealTimingScopeV1.INDIVIDUAL
                }
            val localClusterCarbs =
                sessions.indices
                    .filter { k ->
                        (k == j) ||
                            (k < j &&
                                (sessions[j].startMs - sessions[k].endMs) / 60_000.0 <= 120.0) ||
                            (k > j && (sessions[k].startMs - sessions[j].endMs) / 60_000.0 <= 120.0)
                    }
                    .sumOf { sessions[it].carbsG }
            val corr = x.indices.filter { it != j }.maxOfOrNull { jointCosine(x[j], x[it]) } ?: 0.0
            val close =
                sessions.indices.any { k ->
                    k != j && abs(sessions[k].startMs - sessions[j].startMs) <= 90 * 60_000L
                }
            ...
            val unidentified =
                inputTruncated ||
                    corr > .90 ||
                    close ||
                    times.size < 2 * (sessions.size + 2) ||
                    timingScope == MealTimingScopeV1.CLUSTER_ONLY
            val info = x[j].indices.sumOf { x[j][it] * x[j][it] / (sigma[it] * sigma[it]) }
            val baseWidth =
                (1.2816 * sqrt(1 / (priorStrength[j] + info).coerceAtLeast(1e-9)) * (1 + 4 * corr) +
                    rmse * .30 +
                    prior[j] * gramUncertainty[j] * .35)
            val width = baseWidth.coerceAtLeast(prior[j] * (if (unidentified) .30 else .08))
            val maxFactor = if (priorStrength[j] >= 12) 1.35 else config.mealAmplitudeMaxPrior
            val lo = (beta[j] - width).coerceAtLeast(0.0);
            val hi = (beta[j] + width).coerceAtMost(prior[j] * maxFactor)
            intervalLow[j] = lo;
            intervalHigh[j] = hi
            val mealIdx =
                times.indices.filter {
                    times[it].first >= sessions[j].startMs && times[it].second <= continuousEnds[j]
                };
            val coverage =
                mealIdx.lastOrNull()?.let { (times[it].second - sessions[j].startMs) / 60_000.0 }
                    ?: 0.0
            val expected = (coverage / config.gridMin).toInt().coerceAtLeast(1);
            val density = mealIdx.size.toDouble() / expected
            val available = buildSet {
                if (coverage >= 30) add(ObservedPhaseV1.START);
                if (coverage >= 90) add(ObservedPhaseV1.LEVEL_MAXIMUM);
                if (coverage >= 180) add(ObservedPhaseV1.LATE_PHASE);
                if (coverage >= 240) add(ObservedPhaseV1.PLATEAU)
            }
            ...
            val evidenceCurve = mealIdx.map { i ->
                y[i] -
                    bgBasis[i] * max(bg, 0.0) -
                    process[i] -
                    x.indices.filter { it != j }.sumOf { k -> x[k][i] * max(beta[k], prior[k]) }
            }
            val excursion =
                (evidenceCurve.maxOrNull().orZero() - evidenceCurve.minOrNull().orZero())
            val rawExcursion =
                mealIdx.map { y[it] }.let { it.maxOrNull().orZero() - it.minOrNull().orZero() }
            val phases =
                if (
                    unidentified ||
                        evidenceCurve.size < 3 ||
                        excursion < .20 ||
                        rawExcursion < .20 ||
                        evidenceCurve.maxOrNull().orZero() < .20
                )
                    emptySet()
                else
                    buildSet {
                        val diffs = evidenceCurve.zipWithNext { a, b -> b - a };
                        if (ObservedPhaseV1.START in available && diffs.any { it > .08 })
                            add(ObservedPhaseV1.START)
                        val peak = evidenceCurve.indices.maxByOrNull { evidenceCurve[it] } ?: 0;
                        if (
                            ObservedPhaseV1.LEVEL_MAXIMUM in available &&
                                peak < evidenceCurve.lastIndex
                        )
                            add(ObservedPhaseV1.LEVEL_MAXIMUM)
                        val late = mealIdx.indexOfFirst {
                            (times[it].first - sessions[j].startMs) / 60_000.0 >= 120
                        };
                        if (
                            ObservedPhaseV1.LATE_PHASE in available &&
                                late >= 0 &&
                                evidenceCurve.drop(late).let {
                                    it.isNotEmpty() && it.max() - it.min() > .15
                                }
                        )
                            add(ObservedPhaseV1.LATE_PHASE)
                        val tail = evidenceCurve.takeLast(3);
                        if (
                            ObservedPhaseV1.PLATEAU in available &&
                                tail.size == 3 &&
                                tail.max() - tail.min() <= .20
                        )
                            add(ObservedPhaseV1.PLATEAU)
                    }
            ...
            val censored = coverage < 180 || mealIdx.size < 6 || density < config.minCoverageDensity;
            val status =
                when {
                    inputTruncated -> AttributionResolutionV1.UNRESOLVED;
                    censored -> AttributionResolutionV1.CENSORED;
                    unidentified || fitMismatch -> AttributionResolutionV1.UNRESOLVED;
                    else -> AttributionResolutionV1.RESOLVED
                }
            val next = sessions.getOrNull(j + 1)?.startMs
            fun area(filter: (Long) -> Boolean, basis: DoubleArray = x[j]) =
                times.indices
                    .filter { filter(times[it].first) }
                    .sumOf { basis[it] }
                    .coerceAtLeast(0.0)
            val allArea = area({ true }).coerceAtLeast(1e-9);
            val before = if (next == null) 1.0 else area({ it < next }) / allArea;
            val after = if (next == null) 0.0 else (1 - before).coerceIn(0.0, 1.0)
            val curve =
                times.indices.map { i ->
                    TimeContributionPointV2(
                        times[i].first,
                        beta[j] * x[j][i],
                        lo * xLate[j][i],
                        hi * xEarly[j][i]
            ...
            MealAllocationV1(
                ...
                sessions[j]
                    .notes
                    .mapNotNull { it.carbsSource }
                    .distinct()
                    .joinToString("+")
                    .ifBlank { "unknown" },
                sessions[j].carbsG * (1 - gramUncertainty[j]),
                sessions[j].carbsG * (1 + gramUncertainty[j]),
                prior[j],
                beta[j],
                lo,
                ...
                beta[j] / total,
                lo /
                    (hi + (beta.sum() - beta[j]) + process.maxOrNull().orZero()).coerceAtLeast(
                        1e-9
                    ),
                hi /
                    (lo + (beta.sum() - beta[j]) + process.minOrNull().orZero()).coerceAtLeast(
                        1e-9
                    ),
                status,
                phases,
                available,
                coverage,
                sessions[j].notes.map { it.tsMs },
                ((1 - rmse / 3).coerceIn(.05, 1.0) *
                    (1 - corr).coerceAtLeast(.08) *
                    (if (fitMismatch) .15 else 1.0)),
                buildList {
                    if (unidentified) add("overlap not identifiable");
                    if (timingScope == MealTimingScopeV1.CLUSTER_ONLY)
                        add("individual timing replaced by aggregate meal-cluster timing");
                    if (timingScope == MealTimingScopeV1.COMPLETED_BEFORE_CLUSTER)
                        add("at least 90% completed before next intake; early contribution locked");
                    if (fitMismatch)
                        add("joint model mismatch; episode excluded from confident food learning");
                    if (inputTruncated) add("input truncated; allocation unresolved");
                    if (censored) add("meal-specific CGM window censored at gap or low density");
                    if (phases.isEmpty() && available.isNotEmpty())
                        add("phase window available but phase not established");
                    if (gramUncertainty[j] > .2) add("grams provenance is weak");
                    if (kernels.values.any { it.factors.any { f -> !f.appliedToMedian } })
                        add("insulin uncertainty propagated");
                    if (process.maxOrNull().orZero() > .5) add("shared process residual retained")
                },
                curve,
                PosteriorV1(
                    before,
                    (before - .12).coerceAtLeast(0.0),
                    (before + .12).coerceAtMost(1.0),
                    0,
                    0,
                    asOfMs
                ),
                PosteriorV1(
                    after,
                    (after - .12).coerceAtLeast(0.0),
                    (after + .12).coerceAtMost(1.0),
                    0,
                    0,
                    asOfMs
                ),
                priorStrength[j],
                sessions[j].startMs + (config.sensorLagMedianMin * 60_000).toLong(),
                false,
                timingScope = timingScope,
                clusterCarbsG = localClusterCarbs
            )
        }

    val naive =
        DoubleArray(sessions.size) { j ->
            val from = sessions[j].startMs + (config.sensorLagMedianMin * 60_000).toLong();
            val until = sessions.getOrNull(j + 1)?.startMs ?: intendedEnd
            val ids = times.indices.filter { times[it].first in from..until };
            var num = priorStrength[j] * prior[j];
            var den = priorStrength[j]
            ids.forEach { i ->
                val w = 1 / (sigma[i] * sigma[i]);
                num += w * x[j][i] * (y[i] - bgBasis[i] * bg);
                den += w * x[j][i] * x[j][i]
            }
            (num / den.coerceAtLeast(1e-9)).coerceIn(0.0, prior[j] * config.mealAmplitudeMaxPrior)
        }
    val transfer =
        if (sessions.size < 2) null
        else
            run {
                val last = sessions.lastIndex;
                val deficit = (0 until last).sumOf { (prior[it] - naive[it]).coerceAtLeast(0.0) };
                val surplus = (naive[last] - prior[last]).coerceAtLeast(0.0)
                val lateIds =
                    times.indices.filter {
                        times[it].first >=
                            sessions[last].startMs + (config.sensorLagMedianMin * 60_000).toLong()
                    }
                val previous = lateIds.sumOf { i ->
                    (0 until last).sumOf { j -> x[j][i] * beta[j] }
                };
                val mealAll =
                    lateIds
                        .sumOf { i -> sessions.indices.sumOf { j -> x[j][i] * beta[j] } }
                        .coerceAtLeast(1e-9)
                val residualLate = lateIds.sumOf { process[it] };
                val backgroundLate = lateIds.sumOf { (bgBasis[it] * bg).coerceAtLeast(0.0) };
                val observedParts = (mealAll + residualLate + backgroundLate).coerceAtLeast(1e-9)
                val frac = (previous / mealAll).coerceIn(0.0, 1.0);
                val unassigned = (residualLate / observedParts).coerceIn(0.0, 1.0);
                val unresolved = allocations.any { it.status != AttributionResolutionV1.RESOLVED }
                TailTransferAuditV2(
                    deficit > .3 && surplus > .3,
                    deficit,
                    surplus,
                    ...
                    PosteriorV1(
                        frac,
                        (frac - .18).coerceAtLeast(0.0),
                        (frac + .18).coerceAtMost(1.0),
                        0,
                        0,
                        asOfMs
                    ),
                    PosteriorV1(
                        unassigned,
                        (unassigned - .12).coerceAtLeast(0.0),
                        (unassigned + .12).coerceAtMost(1.0),
                        0,
                        0,
                        asOfMs
                    ),
                    if (unresolved) AttributionResolutionV1.UNRESOLVED
                    else AttributionResolutionV1.RESOLVED,
                    "retrospective constrained joint fit; naive windows are diagnostic only"
                )
            }
    val receiptStatus =
        when {
            allocations.any { it.status == AttributionResolutionV1.UNRESOLVED } ->
                AttributionResolutionV1.UNRESOLVED;
            allocations.any { it.status == AttributionResolutionV1.CENSORED } ->
    ...
    val bins =
        times.indices.map { i ->
            ...
            TimeFitBinV2(
                ...
                bgBasis[i] * bg,
                signedProcess[i],
                pred(i) + errors[i].coerceAtMost(0.0),
                sigma[i]
            )
        }
    val clusterTiming =
        bins
            .maxOfOrNull { it.fittedMeals }
            ?.takeIf { it > .20 }
            ?.let { maximum ->
    ...
    val processCurve =
        times.indices.map { i ->
            TimeContributionPointV2(
                times[i].first,
                signedProcess[i],
                signedProcess[i] - rmse,
                signedProcess[i] + rmse
            )
        }
    val residualPeak = process.maxOrNull().orZero().coerceAtLeast(0.0);
    val observed = y.last() - bgBasis.last() * bg
    val hashBody = buildString {
        append(JOINT_ATTRIBUTION_VERSION_V1)
            .append('|')
            .append(appearance.signature())
            .append('|')
            .append(asOfMs)
            .append('|')
            .append(globalCs)
            ...
            append('|')
                ...
                .append(
                    it.notes.joinToString(",") { n ->
                        val causalAnalysis =
                            n.analysis.takeIf {
                                n.analysisKnownAtMs?.let { known -> known <= asOfMs } == true
                            };
                        "${n.tsMs}/${n.estCarbs}/${n.carbsKnownAtMs}/${n.carbsSource}/$causalAnalysis/${n.analysisKnownAtMs?.takeIf{it<=asOfMs}}"
                    }
                )
        };
        r.forEach { append("|g:${it.tsMs}:${it.mmol}") };
        activeBoluses.forEach {
            append("|b:${it.tsMs}:${it.units}:${it.purpose}:${kernels.getValue(it).provenanceHash}")
        };
        append("|fit:")
            .append(beta.joinToString())
            .append(':')
            .append(bg)
            .append(':')
            .append(process.joinToString())
            .append(':')
            .append(rmse)
    }
    val hash =
        MessageDigest.getInstance("SHA-256").digest(hashBody.toByteArray()).joinToString("") {
            "%02x".format(it)
        }
    val signedSummary = signedProcess.average();
    val signedLow = (signedProcess.minOrNull().orZero() - rmse).coerceAtMost(signedSummary);
    val signedHigh = (signedProcess.maxOrNull().orZero() + rmse).coerceAtLeast(signedSummary)
    return JointAttributionReceiptV1(
        JOINT_ATTRIBUTION_VERSION_V1,
        start,
        intendedEnd,
        asOfMs,
        globalCs,
        ...
        PosteriorV1(
            residualPeak,
            (residualPeak - rmse).coerceAtLeast(0.0),
            residualPeak + rmse,
            0,
            0,
            asOfMs
        ...
        PosteriorV1(
            bg,
            (bg - rmse / 3).coerceAtLeast(-config.backgroundAbsMaxMmolPerHour),
            (bg + rmse / 3).coerceAtMost(config.backgroundAbsMaxMmolPerHour),
            0,
            0,
            asOfMs
        ),
        kernels.mapKeys { it.key.tsMs }.mapValues { it.value.provenanceHash },
        PosteriorV1(observed, observed - rmse, observed + rmse, 0, 0, asOfMs),
        receiptStatus,
        hash,
        bins,
        processCurve,
        transfer,
        inputTruncated,
        PosteriorV1(signedSummary, signedLow, signedHigh, 0, 0, asOfMs),
        clusterTiming
    )
}
