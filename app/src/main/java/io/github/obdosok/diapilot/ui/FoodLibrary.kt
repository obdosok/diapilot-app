        ...
        deconvStats = withContext(Dispatchers.IO) {
            ...
            onsetMed?.let {
                text.resources.getQuantityString(
                    R.plurals.food_library_deconv_stats_with_onset, corpus.size,
                    corpus.size, totalNotes, censoredPct, it, ttpMed, perG,
                )
            } ?: text.resources.getQuantityString(
                R.plurals.food_library_deconv_stats, corpus.size,
                                ...
                                Column(Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                                    ...
                                    if (open) {
                                        ...
                                        if (s.vocabulary.size > 30) {
                                            Text(
                                                pluralStringResource(R.plurals.food_library_and_more, s.vocabulary.size - 30, s.vocabulary.size - 30),
                                                style = MaterialTheme.typography.labelSmall,
                                                color = MaterialTheme.colorScheme.onSurfaceVariant,
                                            )
                                        }
                                ...
                                Column(Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                                    ...
                                    if (open) {
                                        ...
                                        mapped.forEach { p ->
                                            ...
                                            if (p.aliases.size > 8) {
                                                Text(
                                                    "  " + pluralStringResource(R.plurals.food_library_and_more, p.aliases.size - 8, p.aliases.size - 8),
                                                    style = MaterialTheme.typography.labelSmall,
                                                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                                                )
                                            }
            ...
            Column(
                ...
                if (usedIn.isNotEmpty()) {
                    ...
                    Column(verticalArrangement = Arrangement.spacedBy(2.dp)) {
                        ...
                        if (usedIn.size > 6) {
                            Text(
                                pluralStringResource(R.plurals.food_library_and_more, usedIn.size - 6, usedIn.size - 6),
                                style = MaterialTheme.typography.bodySmall,
                                color = MaterialTheme.colorScheme.onSurfaceVariant,
                            )
                        }
                ...
                row.memory?.let { mem ->
                    ...
                    Column(verticalArrangement = Arrangement.spacedBy(2.dp)) {
                        ...
                        if (mem.episodes.size > 8) {
                            Text(
                                pluralStringResource(R.plurals.food_library_and_more, mem.episodes.size - 8, mem.episodes.size - 8),
                                style = MaterialTheme.typography.bodySmall,
                                color = MaterialTheme.colorScheme.onSurfaceVariant,
                            )
                        }
                ...
                Row(
                    ...
                    TextButton(onClick = {
                        ...
                        Thread {
                            ...
                            android.os.Handler(android.os.Looper.getMainLooper()).post {
                                if (renamedOk) onDone()
                                else error = context.localized().getString(R.string.food_library_error_duplicate_name, nn)
                            }
                        }.start()
