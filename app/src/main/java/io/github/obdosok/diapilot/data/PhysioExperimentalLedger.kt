...
object PhysioExperimentalLedger {
     ...
     */
    fun foodEraSourcePrefix(era: com.diapilot.core.api.FoodEra = FoodEraSettings.current()): String =
        "strict-food-era-v3-${era.startMs}:"
    fun isFoodEraState(
        state: PromotionStateV1?,
        era: com.diapilot.core.api.FoodEra = FoodEraSettings.current()
    ): Boolean = state?.sourceEvidenceHash?.startsWith(foodEraSourcePrefix(era)) == true
    data class ShadowCandidate(
        val generationId: String,
        val hypothesisId: String,
        val promotionRevision: Int,
        val evaluationStartKnownAt: Long,
        val baselineArtifactId: String,
    ...
    private fun closedInvalidationTriggers(): List<String> {
        ...
        return specs.flatMap { (table, column) ->
            listOf(
    ...
    fun ensureClosedInvalidationTracking(db: SQLiteDatabase) {
        DDL.filter {
                it.contains("physio_closed_input_invalidations_v1") ||
                    it.contains("physio_closed_invalidation_state_v1")
            }
            .forEach(db::execSQL)
        val existing =
        ...
        closedInvalidationTriggers()
            .filter { sql -> existing.any { sql.contains(" ON $it ") } }
            .forEach(db::execSQL)
    }
    ...
    fun promotionState(db: SQLiteDatabase, id: String, asOfMs: Long): PromotionStateV1? =
        db.rawQuery(
            ...
            .use { c ->
                if (!c.moveToFirst()) null
                    ...
                    PromotionStateV1(
                        id,
                        c.getInt(0),
                        PromotionStageV1.valueOf(c.getString(1)),
                        c.getDouble(2),
                        c.getString(3),
                        c.getLong(4),
                        c.getInt(0).takeIf { it > 1 }?.minus(1),
                        evidence.optJSONObject("typed_effect")?.let(::effectFromJson),
                        evidence.optString("evaluation_hash").takeIf { it.isNotBlank() },
                        evidence.optString("source_evidence_hash").takeIf { it.isNotBlank() }
                    )
                }
            }
    ...
    private fun foodEraPrior(db: SQLiteDatabase, id: String, nowMs: Long): PromotionStateV1? {
        ...
        val next =
            prior.copy(
                ...
                stage = PromotionStageV1.DEMOTED,
                boundedEffectPercent = 0.0,
                effect = null,
                reason = "evidence from outside the current food-era boundary excluded",
                knownAtMs = nowMs,
                evaluationHash = "food-era-reset:$nowMs",
                sourceEvidenceHash = "${prefix}legacy-reset:$nowMs"
            )
        val json =
            org.json
                .JSONObject()
                .put("source_evidence_hash", next.sourceEvidenceHash)
                .put("food_era_reset", true)
                .toString()
        db.execSQL(
            "INSERT OR IGNORE INTO physio_factor_promotion_ledger(hypothesis_id,revision,known_at_ms,stage,bounded_effect_percent,reason,evidence_status,evidence_json) VALUES(?,?,?,?,?,?,?,?)",
            arrayOf<Any?>(
                id,
                next.revision,
                nowMs,
                ...
                0.0,
                next.reason,
                EvidenceStatus.CONFLICTING.name,
                json
            )
        )
        return next
    ...
    fun recordPromotionEvaluation(
        db: SQLiteDatabase,
        definition: HypothesisDefinitionV1,
        evidence: FactorEvidenceV1,
        nowMs: Long,
        prospectiveLossImprovement: Double? = null
    ): PromotionStateV1 {
        val prior = foodEraPrior(db, definition.id, nowMs)
        val sourceHash =
            "${foodEraSourcePrefix()}${evidence.status}:${evidence.rawExposures}:${evidence.usableEpisodes}:${evidence.effectPercent.independentDays}:${evidence.effectPercent.median}:${evidence.effectPercent.p10}:${evidence.effectPercent.p90}:${evidence.typedEffect}"
        if (prior?.sourceEvidenceHash == sourceHash) return prior
        if (prior != null && (evidence.effectPercent.lastUpdateMs ?: nowMs) <= prior.knownAtMs)
            return prior
        if (
            prior?.stage in
                setOf(PromotionStageV1.SHADOW_CANDIDATE, PromotionStageV1.PROSPECTIVE_STABLE) &&
                evidence.status != EvidenceStatus.CONFLICTING
        )
            return prior!!
        if (
            prior?.stage in setOf(PromotionStageV1.PROMOTED_MEDIAN, PromotionStageV1.MONITORED) &&
                evidence.status == EvidenceStatus.SUPPORTED
        )
            return prior!!
        val nextBase =
            PromotionLifecycleV1.next(
                definition.executableSpec,
                evidence,
                prior,
                prospectiveLossImprovement,
                nowMs
            )
        val next =
            nextBase.copy(
                effect =
                    evidence.typedEffect?.let {
                        boundedCandidateEffect(
                            it,
                            definition.executableSpec,
                            evidence.effectPercent.independentDays
                        )
                    },
        ...
        val json =
            org.json
                .JSONObject()
                .put("raw", evidence.rawExposures)
                .put("usable", evidence.usableEpisodes)
                .put("days", evidence.effectPercent.independentDays)
                .put("provenance", evidence.evidenceProvenance.name)
                .put("typed_effect", next.effect?.let(::effectToJson))
                .put("source_evidence_hash", sourceHash)
                .toString()
        db.execSQL(
            "INSERT OR IGNORE INTO physio_factor_promotion_ledger(hypothesis_id,revision,known_at_ms,stage,bounded_effect_percent,reason,evidence_status,evidence_json) VALUES(?,?,?,?,?,?,?,?)",
            arrayOf<Any?>(
    ...
    private fun effectToJson(e: EffectPayloadV1) =
        org.json
            .JSONObject()
    ...
    private fun effectFromJson(o: org.json.JSONObject): EffectPayloadV1 {
        val target = ParameterTargetV1.valueOf(o.getString("target"));
        val m = o.getDouble("median");
        val l = o.getDouble("low");
        val h = o.getDouble("high")
        return when (target.unit) {
            "fraction" -> EffectPayloadV1.Fraction(target, m, l, h);
            "minute" -> EffectPayloadV1.Minutes(target, m, l, h);
            "mmol/L/hour" -> EffectPayloadV1.Rate(target, m, l, h);
            "mmol/L" -> EffectPayloadV1.Bias(target, m, l, h);
            else -> error("bad unit")
        }
    }
    private fun effectPercent(e: EffectPayloadV1) = e.median / e.target.displayReference * 100.0
    private fun boundedCandidateEffect(
        e: EffectPayloadV1,
        spec: ExecutableHypothesisSpecV1,
        days: Int
    ): EffectPayloadV1 {
        val raw = effectPercent(e);
        val allowed =
            raw.coerceIn(-spec.maxMedianEffectPercent, spec.maxMedianEffectPercent) *
                (days.toDouble() / spec.rampInDays).coerceIn(0.0, 1.0)
    ...
    fun recordObservationalCandidate(
        db: SQLiteDatabase,
        definition: HypothesisDefinitionV1,
        evidence: FactorEvidenceV1,
        nowMs: Long
        ...
        require(
            definition.executableSpec.implementationStatus == ImplementationStatusV1.EXECUTABLE &&
                definition.executableSpec.applicationPolicy != ApplicationPolicyV1.ATTRIBUTION_ONLY
        )
        ...
        require(
            evidence.usableEpisodes >= definition.executableSpec.minimumIndependentEpisodes &&
                evidence.effectPercent.independentDays >=
                    definition.executableSpec.minimumIndependentDays
        )
        val effect =
            evidence.typedEffect ?: error("observational candidate needs typed bounded payload")
        val sourceHash =
            "${foodEraSourcePrefix()}observational:${evidence.rawExposures}:${evidence.usableEpisodes}:${evidence.effectPercent.independentDays}:${evidence.effectPercent.median}:${effect}"
        foodEraPrior(db, definition.id, nowMs)
            ?.takeIf { it.sourceEvidenceHash == sourceHash }
            ?.let {
                return it
            }
        val prior = foodEraPrior(db, definition.id, nowMs)
        ...
        val measuredBase =
            definition.executableSpec.applicationPolicy == ApplicationPolicyV1.MEASURED_BASE
        if (
            !measuredBase &&
                prior?.stage in
                    setOf(
                        PromotionStageV1.SHADOW_CANDIDATE,
                        PromotionStageV1.PROSPECTIVE_STABLE,
                        PromotionStageV1.PROMOTED_MEDIAN,
                        PromotionStageV1.MONITORED
                    )
        )
            return prior!!
        val bounded =
        ...
        val stage =
            if (measuredBase) PromotionStageV1.PROMOTED_MEDIAN else PromotionStageV1.SHADOW_CANDIDATE
        val next =
            PromotionStateV1(
                definition.id,
                (prior?.revision ?: 0) + 1,
                ...
                if (measuredBase)
                    "direct measurement of the base parameter; capped, ramped by independent days, decayed"
        ...
        val json =
            org.json
                ...
                .put("days", evidence.effectPercent.independentDays)
                .put("provenance", evidence.evidenceProvenance.name)
                .put("typed_effect", effectToJson(bounded))
                .put("source_evidence_hash", sourceHash)
                .toString()
        db.execSQL(
            "INSERT OR IGNORE INTO physio_factor_promotion_ledger(hypothesis_id,revision,known_at_ms,stage,bounded_effect_percent,reason,evidence_status,evidence_json) VALUES(?,?,?,?,?,?,?,?)",
    ...
    fun revokeObservationalCandidate(
        db: SQLiteDatabase,
        id: String,
        nowMs: Long,
        reason: String
        ...
        val prior = promotionState(db, id, nowMs) ?: return null
        val provisional =
            prior.stage in setOf(PromotionStageV1.SHADOW_CANDIDATE, PromotionStageV1.PROSPECTIVE_STABLE)
        ...
        val next =
            prior.copy(
                ...
                reason = reason,
                knownAtMs = nowMs,
                evaluationHash = "revoked:$nowMs",
                sourceEvidenceHash = "revoked:$nowMs"
            )
        val json =
            org.json
                .JSONObject()
    ...
    fun demoteIfStale(
        ...
        if (prior.stage !in setOf(PromotionStageV1.PROMOTED_MEDIAN, PromotionStageV1.MONITORED))
            return prior
        ...
        val next =
            prior.copy(
                ...
                stage = PromotionStageV1.DEMOTED,
                boundedEffectPercent = 0.0,
                reason = "automatic demotion: evidence stale beyond three half-lives",
                knownAtMs = nowMs,
                evaluationHash = "stale:$nowMs"
            )
        val json =
            org.json
                .JSONObject()
                .put("typed_effect", prior.effect?.let(::effectToJson))
                .put("evaluation_hash", next.evaluationHash)
                .put("source_evidence_hash", prior.sourceEvidenceHash)
                .toString()
        db.execSQL(
            "INSERT OR IGNORE INTO physio_factor_promotion_ledger(hypothesis_id,revision,known_at_ms,stage,bounded_effect_percent,reason,evidence_status,evidence_json) VALUES(?,?,?,?,?,?,?,?)",
            arrayOf<Any?>(
                ...
                0.0,
                next.reason,
                EvidenceStatus.INSUFFICIENT.name,
                json
            )
        )
        return next
