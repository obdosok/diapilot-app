...
data class FoodCalculationV1(
    ...
    val model: String,
    val carbsG: Double,
    val carbsProvenance: String,
    val proteinG: Double?,
    val fatG: Double?,
    val durationMin: Double,
    val globalCsMedian: Double,
    val globalCsLow: Double,
    val globalCsHigh: Double,
    val totalAmplitudeMmol: Double,
    val timingSource: String,
    val timingTemplateId: String,
    val onsetMin: Double,
    val ratePeakMin: Double,
    val effectEndMin: Double,
    val timingUncertaintyMin: Double,
    val foodContribution60: Double,
    val foodContribution180: Double,
    val insulinContribution60: Double,
    val backgroundContribution60: Double,
    val isfMedian: Double,
    val isfLow: Double,
    val isfHigh: Double,
    val uncertaintySummary: String,
    val globalCsOnlineEpisodes: Int = 0,
    val globalCsOnlineDays: Int = 0,
...
data class EpisodeAttributionExplanationV1(
    ...
    val modelForecast: String,
    val allocatedMealContribution: String,
    val phases: String,
    val overlap: String,
    val allocation: String,
    val resolution: String,
    val episodeKernel: String,
    val kernelDifference: String,
    val confidence: String,
    val caveats: String,
    val causalProvenance: String = "",
    val diagnosticProvenance: String = "",
    val timeResolvedCurve: String = "",
    val tailTransfer: String = "",
    val compactSummary: String = "",
    val compactFinding: String = "",
...
object FoodCalculationRegistry {
    ...
    fun sourceKey(store: SqliteCollectorStore, context: Context, nowMs: Long): String =
        store.stage10ReceiptCacheKey(Stage9EpisodeRuntime.CACHE_VERSION, nowMs) +
            "|ui=" +
            context.uiLanguage()
    ...
    @Synchronized
    fun update(next: Map<Long, FoodCalculationV1>) {
        baseValues = next.toMap();
        values = baseValues.mapValues { (id, v) -> decorated(id, v) }
    }
    @Synchronized
    fun updateEpisodeAttribution(
        next: Map<Long, EpisodeAttributionExplanationV1>,
        generation: Long = maxOf(expectedGeneration, mutableEpisode.value.generation) + 1,
        complete: Boolean = true,
        processedClusters: Int = 0,
        totalClusters: Int = 0,
        budgetLimited: Boolean = false,
    ): Boolean {
        if (generation < expectedGeneration || generation < mutableEpisode.value.generation)
            return false
        val receipts = if (complete) next.toMap() else mutableEpisode.value.receipts + next
        mutableEpisode.value =
            EpisodeSnapshot(
                generation,
                receipts,
                complete,
    ...
    @Synchronized
    fun updateEpisodeAttributionWindow(
        next: Map<Long, EpisodeAttributionExplanationV1>,
        replaceIds: Set<Long>,
        generation: Long,
        runComplete: Boolean,
        ...
        mutableEpisode.value =
            EpisodeSnapshot(
                ...
                if (runComplete) 0 else nextOffset,
                nowMs,
            )
        values = baseValues.mapValues { (id, v) -> decorated(id, v) }
        return true
     ...
     */
    @Synchronized
    fun restore(context: Context, key: String): Boolean {
        ...
        val root =
            runCatching { JSONObject(File(context.filesDir, DISK_FILE).readText()) }.getOrNull()
                ?: return false
        if (
            root.optInt("schema") != DISK_SCHEMA ||
                root.optString("algorithmVersion") != Stage9EpisodeRuntime.CACHE_VERSION
        )
            return false
        ...
        val receipts =
            buildMap<Long, EpisodeAttributionExplanationV1> {
        ...
        mutableEpisode.value =
            EpisodeSnapshot(
                generation = 0,
                receipts = receipts,
                complete = root.optBoolean("complete", true),
                processedClusters = root.optInt("processedClusters"),
                totalClusters = root.optInt("totalClusters"),
                budgetLimited = root.optBoolean("budgetLimited"),
                fullRefreshAtMs = root.optLong("fullRefreshAtMs"),
                nextOffset = root.optInt("nextOffset"),
                lastAttemptAtMs = root.optLong("lastAttemptAtMs"),
            )
    ...
    @Synchronized
    fun persist(context: Context, key: String): Boolean {
    ...
    private fun persistSnapshot(context: Context, key: String, snapshot: EpisodeSnapshot): Boolean {
        val root =
            JSONObject()
                .put("schema", DISK_SCHEMA)
                .put("algorithmVersion", Stage9EpisodeRuntime.CACHE_VERSION)
                .put("sourceKey", key)
                .put("complete", snapshot.complete)
                .put("processedClusters", snapshot.processedClusters)
                .put("totalClusters", snapshot.totalClusters)
                .put("budgetLimited", snapshot.budgetLimited)
                .put("fullRefreshAtMs", snapshot.fullRefreshAtMs)
                .put("nextOffset", snapshot.nextOffset)
                .put("lastAttemptAtMs", snapshot.lastAttemptAtMs)
                .put("language", receiptsLanguage ?: context.uiLanguage())
                .put(
                    "receipts",
                    JSONObject().apply {
                        snapshot.receipts.forEach { (id, e) ->
                            put(id.toString(), explanationToJson(e))
                        }
                    }
                )
        val target = File(context.filesDir, DISK_FILE);
        val tmp = File(context.filesDir, "$DISK_FILE.tmp")
        return runCatching {
            tmp.writeText(root.toString());
            if (target.exists() && !target.delete()) error("cannot replace Stage10 cache")
    ...
    fun isCurrent(key: String) =
        sourceKey == key && mutableEpisode.value.complete && mutableEpisode.value.receipts.isNotEmpty()
    fun needsFullRefresh(nowMs: Long, maxAgeMs: Long = 24 * 60 * 60_000L): Boolean =
        mutableEpisode.value.fullRefreshAtMs <= 0 ||
            nowMs - mutableEpisode.value.fullRefreshAtMs >= maxAgeMs
     ...
     */
    @Synchronized
    fun requestContinuationBudget(budgetMs: Long = 8_000L) {
        requestedContinuationBudgetMs =
            maxOf(requestedContinuationBudgetMs, budgetMs.coerceIn(2_000L, 15_000L))
    }
    @Synchronized
    fun takeContinuationBudget(defaultMs: Long = 2_000L): Long {
    ...
    @Synchronized
    fun invalidateClosedEpisodeEligibility(
        ids: Set<Long>,
        ...
        val barrier =
            maxOf(proposedGeneration, expectedGeneration + 1, mutableEpisode.value.generation + 1)
        expectedGeneration = barrier
        closedEligibilityBlocked = ids.toSet()
        mutableEpisode.value =
            previous.copy(
                generation = barrier,
                receipts = previous.receipts.filterKeys { it !in ids },
                complete = false
            )
        sourceKey = null
        values = baseValues.mapValues { (id, v) -> decorated(id, v) }
        ...
        context?.let {
            persistSnapshot(it, "closed-eligibility-invalidated:$barrier", mutableEpisode.value)
        }
        return barrier
    }

    private fun explanationToJson(e: EpisodeAttributionExplanationV1) =
        JSONObject()
            .put("receiptVersion", e.receiptVersion)
            .put("modelForecast", e.modelForecast)
            .put("allocatedMealContribution", e.allocatedMealContribution)
            .put("phases", e.phases)
            .put("overlap", e.overlap)
            .put("allocation", e.allocation)
            .put("resolution", e.resolution)
            .put("episodeKernel", e.episodeKernel)
            .put("kernelDifference", e.kernelDifference)
            .put("confidence", e.confidence)
            .put("caveats", e.caveats)
            .put("causalProvenance", e.causalProvenance)
            .put("diagnosticProvenance", e.diagnosticProvenance)
            .put("unloggedResidualMmol", e.unloggedResidualMmol)
            .put("timeResolvedCurve", e.timeResolvedCurve)
            .put("tailTransfer", e.tailTransfer)
            .put("compactSummary", e.compactSummary)
            .put("compactFinding", e.compactFinding)
            .put("unresolved", e.unresolved)

    private fun explanationFromJson(o: JSONObject) =
        EpisodeAttributionExplanationV1(
            receiptVersion = o.optString("receiptVersion"),
            modelForecast = o.optString("modelForecast"),
            allocatedMealContribution = o.optString("allocatedMealContribution"),
            phases = o.optString("phases"),
            overlap = o.optString("overlap"),
            allocation = o.optString("allocation"),
            resolution = o.optString("resolution"),
            episodeKernel = o.optString("episodeKernel"),
            kernelDifference = o.optString("kernelDifference"),
            confidence = o.optString("confidence"),
            caveats = o.optString("caveats"),
            causalProvenance = o.optString("causalProvenance"),
            diagnosticProvenance = o.optString("diagnosticProvenance"),
            unloggedResidualMmol = o.optDouble("unloggedResidualMmol", 0.0),
            timeResolvedCurve = o.optString("timeResolvedCurve"),
            tailTransfer = o.optString("tailTransfer"),
            compactSummary = o.optString("compactSummary"),
            compactFinding = o.optString("compactFinding"),
            unresolved = o.optBoolean("unresolved"),
        )
    @Synchronized
    fun expectEpisodeGeneration(generation: Long) {
        if (generation > expectedGeneration) expectedGeneration = generation
    }
    @Synchronized
    internal fun resetEpisodeStateForTest() {
        expectedGeneration = 0;
        sourceKey = null;
        restoreAttemptedKey = null;
        closedEligibilityBlocked = emptySet();
        requestedContinuationBudgetMs = 0;
