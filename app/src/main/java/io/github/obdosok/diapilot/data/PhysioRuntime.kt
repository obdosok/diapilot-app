...
object PhysioRuntime {
    ...
    fun artifact(
        ...
        val key =
            sqlite?.let { db ->
                ...
                val closed =
                    db.rawQuery(
                            "SELECT COALESCE(MAX(known_at_ms),0),COUNT(*) FROM physio_closed_episodes",
                            null,
                        )
        ...
        return buildArtifact(store, asOfMs).also {
            ...
            if (ms >= 200)
                android.util.Log.i(
                    "PhysioRuntime",
                    "artifact rebuilt in $ms ms — changed: $changed",
                )
        }
    }
    ...
    private fun buildArtifact(
        ...
        HybridShadowRegistry.model()?.let { base ->
            ...
            val safeFoodPeak =
                shape.peakMin.coerceIn(
                    maxOf(safeDelay + 1, bounds.absorptionPeakMinRange.start)..bounds
                            .absorptionPeakMinRange
                            .endInclusive
                )
            val safeFoodTail =
                shape.durationMin.coerceIn(
                    maxOf(safeFoodPeak + 1, bounds.absorptionTailMinRange.start)..bounds
                            .absorptionTailMinRange
                            .endInclusive
                )
            val safeBase =
                base.copy(
                    insulin =
                        base.insulin.copy(
                            isf =
                                base.insulin.isf.coerceIn(
                                    bounds.isfMmolPerLUmin,
                                    bounds.isfMmolPerLUmax
                                ),
                            isfLow =
                                base.insulin.isfLow.coerceIn(
                                    bounds.isfMmolPerLUmin,
                                    bounds.isfMmolPerLUmax
                                ),
                            isfHigh =
                                base.insulin.isfHigh.coerceIn(
                                    bounds.isfMmolPerLUmin,
                                    bounds.isfMmolPerLUmax
                                ),
                            onsetMin = safeOnset,
                            peakMin = safePeak,
                            shortDurationMin = safeShort,
                            tailDurationMin = safeTail
                        ),
                    food =
                        base.food.copy(
                            defaultShape =
                                shape.copy(
                                    delayMin = safeDelay,
                                    peakMin = safeFoodPeak,
                                    durationMin = safeFoodTail
                                )
                        )
                )
            val sqlite = (store as? SqliteCollectorStore)?.readableDatabase
            ...
            val profileState = sqlite?.let {
                runCatching { InsulinProfileRuntime.state(store!!, it, asOfMs) }.getOrNull()
            }
                    ...
                    "segments: doses ${st.dosesConsidered}" +
                        (p?.let {
                            " · arm ${it.arm}" +
                                ...
                                " · applied slowdown %.0f (n=%d) · end %.0f (n=%d)"
                                    .format(
                                        it.slowdown?.minute ?: -1.0,
                                        it.slowdown?.samples ?: 0,
                                        it.tailEnd?.minute ?: -1.0,
                                        it.tailEnd?.samples ?: 0,
                                    ) +
                                ...
                                (it.slowdownConditioned?.let { c ->
                                    " · [slowdown if conditioned] no food %.0f (n=%d) / with food %.0f (n=%d) / all %.0f (n=%d)"
                                        .format(
                                            c.flat?.minute ?: -1.0,
                                            c.flat?.samples ?: 0,
                                            c.rising?.minute ?: -1.0,
                                ...
                                (it.tailConditioned?.let { c ->
                                    " · [end if conditioned] no food %.0f (n=%d) / with food %.0f (n=%d) / all %.0f (n=%d)"
                                        .format(
                                            c.flat?.minute ?: -1.0,
                                            c.flat?.samples ?: 0,
                                            c.rising?.minute ?: -1.0,
            ...
            val adaptiveIsf =
                IsfSource.adaptiveInForce(manualContext)
                    ?.coerceIn(bounds.isfMmolPerLUmin, bounds.isfMmolPerLUmax)
            val manualParams =
                ManualInsulinRuntime.params(manualContext).let {
                    if (adaptiveIsf != null) it.copy(isfMmolPerU = null) else it
                }
            ...
            android.util.Log.i(
                ...
                "manual params: shape=${resolution.shapeTier} ISF=${resolution.isfTier}" +
                    ...
                    else " · REJECTED: ${resolution.rejected.joinToString(", ")}") +
                    (if (resolution.divergences.isEmpty()) ""
                    else " · divergences: ${resolution.divergences.size}"),
            )
            val mechanics =
                ManualInsulinRuntime.apply(widened, resolution).let { m ->
                        ...
                        m.copy(
                            insulin =
                                m.insulin.copy(
                                    ...
                                    isfHigh =
                                        (m.insulin.isfHigh * scale).coerceAtLeast(
                                            m.insulin.isfLow * scale
                                        ),
                                )
                        )
                    }
                }
            val mechanicsCoerced =
                safeBase.insulin != base.insulin || safeBase.food.defaultShape != base.food.defaultShape
            ...
            val selected =
                (store as? SqliteCollectorStore)
                    ?.readableDatabase
                    ?.rawQuery(
                        "SELECT checkpoint_id,known_at_ms,payload_json FROM physio_daily_checkpoints WHERE known_at_ms>=? AND known_at_ms<=? ORDER BY known_at_ms DESC,checkpoint_id DESC LIMIT 1",
                        arrayOf(FoodEraSettings.current().startMs.toString(), asOfMs.toString()),
                    )
                    ?.use { c ->
                        if (c.moveToFirst()) Triple(c.getString(0), c.getLong(1), c.getString(2))
                        else null
                    }
            val identified = selected?.third?.let { org.json.JSONObject(it) }
            fun parsedPosterior(name: String, defaultHalf: Double): PosteriorV1 {
                val p =
                    identified?.optJSONObject(name)
                        ?: return PosteriorV1(0.0, -defaultHalf, defaultHalf, 0, 0, null)
                return PosteriorV1(
                    p.getDouble("median"),
                    p.getDouble("p10"),
                    p.getDouble("p90"),
            ...
            val storedState =
                DailyResponseStateV1(
                    parsedPosterior("effective", 30.0),
                    parsedPosterior("identified_isf", 20.0),
                    identified
                        ?.optString("isf_status")
                        ?.takeIf { it.isNotBlank() }
                        ?.let { EvidenceStatus.valueOf(it) } ?: EvidenceStatus.INSUFFICIENT
                )
            val transitioned =
                if (selected != null && asOfMs != Long.MAX_VALUE)
                    DailyResponseEstimatorV1.carryForward(storedState, asOfMs)
                else storedState
            ...
            val canUpdateIsf =
                nIsf >= 6 && isfPct.independentDays >= 3 && isfStatus == EvidenceStatus.SUPPORTED.name
            ...
            val conflicts = buildList {
                ...
                if (baseCsRaw != baseCsBounded)
                    add(
                        "legacy global CS prior $baseCsRaw coerced to physiological bound $baseCsBounded"
                    )
                if (mechanicsCoerced)
                    add("legacy mechanics coerced to PHYSIO bounds; model/data conflict")
                ...
                if (InsulinPriorV1.isPrior(mechanics))
                    add(
                ...
                personalCurve
                    ...
                    ?.let {
                        add(
                            "measured curve coerced to bounds: ${it.joinToString(", "){c->"%s %.0f→%.0f".format(java.util.Locale.ROOT,c.landmark.name,c.fromMin,c.toMin)}}"
                        )
                    }
            }
            fun scaled(pct: Double): Double =
                (mechanics.insulin.isf * (1.0 + pct / 100.0)).coerceIn(
                    bounds.isfMmolPerLUmin,
                    bounds.isfMmolPerLUmax
                )
            ...
            val learnedIsfPosterior =
                if (canUpdateIsf)
                    PosteriorV1(
                        scaled(isfPct.median),
                        minOf(scaled(isfPct.p10), scaled(isfPct.median)),
                        maxOf(scaled(isfPct.p90), scaled(isfPct.median)),
                        nIsf,
                        isfPct.independentDays,
                        asOfMs,
                    )
                else null
            ...
            val heldPosterior =
                PosteriorV1(
                    mechanics.insulin.isf,
                    minOf(mechanics.insulin.isfLow, mechanics.insulin.isf),
                    maxOf(mechanics.insulin.isfHigh, mechanics.insulin.isf),
                    0,
                    0,
                    null
                )
            ...
            val promotionIdentity =
                promoted
                    .sortedBy { it.hypothesisId }
                    .joinToString("|") {
                        "${it.hypothesisId}:${it.promotionRevision}:${it.effect.target}:${it.effect.median}:${it.evaluationHash}"
                    }
            ...
            val variance =
                varianceWithPromotions(
                    com.diapilot.core.physio.PHYSIO_BASE_VARIANCE_HEALTHY_V1,
                    promoted
                )
            PhysioArtifactV1(
                artifactId(
                    (selected?.let {
                        "${it.first}|${it.second}|${it.third}|asof=$asOfMs|transitioned=$isfPct"
                    } ?: "prior-only") +
                        "|promotions=$promotionIdentity|tuning=$tuningIdentity|safe=${mechanics.insulin}:${mechanics.food.defaultShape}|personal-cdf=${personalCurve?.observations}/${personalCurve?.independentDays}|food=macro-mixture-v3-no-dish-curves|cs=$baseCs"
                ),
                mechanics,
                PosteriorV1(
                    baseCs,
    ...
    private fun varianceWithPromotions(
        base: VarianceLedgerV1,
        mods: List<AppliedModifierV1>,
        globalCsEffect: EffectPayloadV1? = null
    ): VarianceLedgerV1 {
        ...
        fun extra(vararg targets: ParameterTargetV1) =
            kotlin.math.sqrt(
                mods
                    .filter { it.effect.target in targets }
