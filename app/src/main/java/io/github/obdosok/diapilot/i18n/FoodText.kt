...
object FoodText {
    ...
    /**
     * A meal label for display. The four [SysLabels] are STORED keys
     * ([SysLabels.DAWN], ...; older builds stored Russian words) and are shown in
     * the UI language; a tag-shaped label ("dextrose ×2") goes through
     * [TokenText.noteTag]; any other label is the user's own text and comes
     * back unchanged.
     */
    fun mealLabel(context: Context, label: String): String {
        val id = when (SysLabels.keyOf(label)) {
            ...
            else -> return TokenText.noteTag(context, label)
        }
