...
object TwinCache {
    ...
    fun get(store: CollectorStore, context: android.content.Context): Model? {
        ...
        val corridors = com.diapilot.core.twin.calibrateRegimeCorridors(
            ...
            foods = com.example.diapilot.data.FoodSources.historicalFoods(
        ...
        val confounderWindows = buildList {
            annotationsAll.forEach { n ->
                val text = n.content.lowercase()
                when {
                    text in setOf("болею", "стресс", "алкоголь") ->
        ...
        val softConfounderWindows = buildList {
            ...
            annotationsAll.forEach { n ->
                if (n.content.lowercase() in setOf("тренировка", "прогулка")) {
                    add(com.diapilot.core.analysis.FoodContaminationWindow(n.tsMs - 60L * 60_000, n.tsMs + 6L * 3_600_000))
                }
            }
        }
        ...
        val rescueWindows = annotationsAll.mapNotNull { n ->
            if (n.content.lowercase().startsWith(com.diapilot.core.analysis.RESCUE_NOTE_PREFIX)) {
