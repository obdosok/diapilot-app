...
internal data class UiState(
    ...
    val hybridWhatIfProfile: com.example.diapilot.data.HybridWhatIfProfile? = null,
    val comparisonWhatIfProfile: com.example.diapilot.data.HybridWhatIfProfile? = null,
    ...
    val hybridFoodReadouts: Map<Long, com.example.diapilot.data.HybridFoodReadout> = emptyMap(),
...
internal fun loadLivePreview(
    ...
    val last = com.example.diapilot.data.CalibratedGlucose.lastReading(store, context)
    ...
    val chart = com.example.diapilot.data.displayHistory(store, context, from, now)
    ...
    val stored = previous.prediction.ifEmpty {
        (store as? com.example.diapilot.data.SqliteCollectorStore)?.let { sqlite ->
            runCatching {
                com.example.diapilot.data.ForecastLedger.lastShownPrediction(
...
internal const val DEXTROSE_PREFIX = "декстроза"
...
internal suspend fun loadState(
    ...
    val meterCalTimeline = com.example.diapilot.data.MeterCalCache.timeline(store, meterCal)
    ...
    val model = if (allowBuild) timed("twin build (allowBuild)") {
        com.example.diapilot.data.TwinCache.get(store, context)
    ...
    com.example.diapilot.data.HybridRuntimeMetrics.installLiveFoodObservations(
    ...
    val currentIob = timed("  iob surface") {
        com.example.diapilot.data.HybridRuntimeMetrics.surfaceIobUnits(store, context, now)
    }
    ...
    val forecastResult = if (last != null && now - last.tsMs <= 36L * 3_600_000 && model != null) {
        timed("  Forecaster.forecast") { com.example.diapilot.data.Forecaster.forecast(
            store, model, now,
    ...
    val chartEffectsTo = if (
        ...
        maxOf(
            ...
            com.example.diapilot.data.HybridRuntimeMetrics
                .activeInsulinEndMs(store, now, context) ?: now,
            com.example.diapilot.data.HybridRuntimeMetrics.activeFoodEndMs(store, now) ?: now,
        )
    } else chartToBound
    lap("live forecast")
    ...
    val labeledAll500 = store.labeledMeals(limit = 500)
        .filter { it.event.onsetMs >= com.example.diapilot.data.FoodEraSettings.current().startMs }
    ...
    val foodSuggestions = run {
        val dismissed = com.example.diapilot.data.Settings.foodSuggestDismissed(context)
    ...
    val selectedWhatIfProfile = whatIfAnchor
        ?.let(com.example.diapilot.data.HybridShadowRegistry::physioWhatIf)
    val chartBolusRows=store.bolusesAll(chartWindow,chartToBound)
    val physioForCounterfactual=com.example.diapilot.data.PhysioRuntime.artifact(store,now)
    val counterfactualPeople=chartBolusRows.filter{it.units>0&&!it.purpose.equals("воздух",true)}.mapNotNull{b->
    ...
    val result = UiState(
        ...
        chartReadings = timed("chartReadings") {
            val pts = com.example.diapilot.data.displayHistory(
        ...
        iobSeries = timed("iobSeries") {
            com.example.diapilot.data.HybridRuntimeMetrics.surfaceIobSeries(
        ...
        cobSeries = timed("cobSeries") {
            com.example.diapilot.data.HybridRuntimeMetrics.physioCobSeries(store, chartWindow, chartEffectsTo)
        },
        ...
        carbsByFood = outlined {
            ...
            val library = (store as? com.example.diapilot.data.SqliteCollectorStore)
                ?.foodLibrary().orEmpty()
                .mapNotNull { e ->
        ...
        recentFoodTexts = annAll
            .filter {
                it.kind == "food" && it.content.isNotBlank() &&
                    !it.content.startsWith(DEXTROSE_PREFIX) &&
        ...
        mealComponents = outlined {
            ...
            val fromLibrary = (store as? com.example.diapilot.data.SqliteCollectorStore)
                ?.foodLibrary().orEmpty()
        ...
        compositeParts = (store as? com.example.diapilot.data.SqliteCollectorStore)
            ?.foodLibrary().orEmpty()
            .filter { it.components.isNotEmpty() }
            .mapNotNull { e ->
                val key = com.diapilot.core.analysis.normalizeFoodName(e.name)
        ...
        hcGranted = if (com.example.diapilot.data.HealthConnectSync.clientOrNull(context) == null) null
        else com.example.diapilot.data.HealthConnectSync.hasPermissions(context),
        stepsGranted = if (com.example.diapilot.data.HealthConnectSync.clientOrNull(context) == null) null
        else com.example.diapilot.data.HealthConnectSync.hasStepsPermission(context),
        ...
        calibrationWindow = outlined {
            ...
            com.diapilot.core.analysis.detectCalibrationWindow(
                ...
                rangeLo = com.example.diapilot.data.Settings.rangeLoMmol(context),
                rangeHi = com.example.diapilot.data.Settings.rangeHiMmol(context),
            )
        },
        ...
        historyReadings = if (includeHistory) timed("historyReadings") {
            ...
            com.example.diapilot.data.displayHistory(store, context, historyFrom, historyTo)
        ...
        foodMemory = if (!includeHistory) emptyMap() else timed("foodMemory") {
            val labeledAll = labeledAll500
            val allBoluses = store.boluses(com.example.diapilot.data.FoodEraSettings.current().startMs, now)
            val allNotes = annAll.filter { it.tsMs >= com.example.diapilot.data.FoodEraSettings.current().startMs }
        ...
        similarFood = outlined {
            ...
            val lib = (store as? com.example.diapilot.data.SqliteCollectorStore)
                ?.foodLibrary().orEmpty()
            ...
            val componentsByDish = dishNames.associateWith { n ->
                recipeByName[n]
                    ?: com.example.diapilot.ui.parseMealParts(n)
        ...
        labeled = store.labeledMeals(limit = 20).filter { it.event.onsetMs >= com.example.diapilot.data.FoodEraSettings.current().startMs },
        lastReading = last,
        ...
        rangeLo = com.example.diapilot.data.Settings.rangeLoMmol(context),
        targetMmol = com.example.diapilot.data.Settings.targetMmol(context),
        rangeHi = com.example.diapilot.data.Settings.rangeHiMmol(context),
        ...
        carbSensMmolPerGram =
            if (true) {
                com.example.diapilot.data.HybridRuntimeMetrics.carbSensitivityMmolPerGram()
            } else {
                ...
                if(artifact==null)emptyMap() else {
                    (store as? com.example.diapilot.data.SqliteCollectorStore)?.let { sqlite ->
                        ...
                        val projectionIdentity = listOf(
                            ...
                            com.example.diapilot.BuildConfig.VERSION_NAME,
                        ...
                        com.example.diapilot.data.HistoryFoodProjectionCache.getOrCompute(
                            sqlite, projectionIdentity, notes,
                        ) { missing ->
                            com.example.diapilot.data.HybridRuntimeMetrics.foodReadoutsForModel(
        ...
        insulinDiaMin =
            com.example.diapilot.data.HybridRuntimeMetrics.physioInsulinLandmarks(store,now)?.effectEndMin
                ?: com.example.diapilot.data.Settings.insulinDiaMin(context),
        insulinPeakMin =
            com.example.diapilot.data.HybridRuntimeMetrics.physioInsulinLandmarks(store,now)?.ratePeakMin
                ?: com.example.diapilot.data.Settings.insulinPeakMin(context),
        ...
        dayStats = timed("dayStats") {
            val day = com.example.diapilot.data.trustedHistory(
                store, context, now - 24L * 3_600_000, now,
            )
            if (day.isEmpty()) null else {
                val lo = com.example.diapilot.data.Settings.rangeLoMmol(context)
                val hi = com.example.diapilot.data.Settings.rangeHiMmol(context)
                ...
                val tir = com.example.diapilot.data.tir24h(store, context, now, lo, hi)
                DayStats(
        ...
        hypoTablets = store.annotations(now - 30 * 60_000, now)
            .firstOrNull { it.content.startsWith(DEXTROSE_PREFIX) }
        ...
        statusLines = timed("statusLines") {
            ...
            com.diapilot.core.analysis.statusSummary(
                com.diapilot.core.analysis.StatusInput(
                    ...
                    cobGrams = com.example.diapilot.data.HybridRuntimeMetrics
    ...
    if(allowBuild) (store as? com.example.diapilot.data.SqliteCollectorStore)?.let { sqlite ->
        UiMaintenanceQueue.schedule {
            ...
            stage("dropRetiredLedgers"){com.example.diapilot.data.LedgerRetention.dropRetiredLedgers(context,sqlite)}
            ...
            stage("ledgerRetention"){com.example.diapilot.data.LedgerRetention.runIfDue(context,sqlite)}
            stage("ledgerReclaim"){com.example.diapilot.data.LedgerRetention.reclaimOnce(context,sqlite)}
            stage("dailyDiscrepancy"){com.example.diapilot.data.DailyDiscrepancyRuntime.update(store,db,now)}
            stage("adaptiveIsf"){
                com.example.diapilot.data.AdaptiveIsfRuntime.refreshIfDue(context,store,now)
            }
