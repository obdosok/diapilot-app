...
class WatchServer(private val context: Context) {
    ...
    private fun handle(client: Socket) {
        ...
        if (route == com.diapilot.core.api.LocalRoute.INFO_JSON) params["newer_than"]?.toLongOrNull()?.let { since ->
            ...
            while (System.currentTimeMillis() < deadline) {
                ...
                val mts = if (com.example.diapilot.data.MinuteCalCache.get(store, context) != null) {
        ...
        val (status, body) = when (route) {
            ...
            com.diapilot.core.api.LocalRoute.EVENTS -> {
                ...
                val r = com.example.diapilot.api.DiaForApi.handle(context, params)
    ...
    private fun infoJson(includeGraph: Boolean = false): String {
        ...
        val bg = JSONObject()
            ...
            .put("dim", com.example.diapilot.data.Settings.watchNightDimActive(context))
            .put("time", last.tsMs)
    ...
    private fun graphJson(
        ...
        val readings = com.example.diapilot.data.displayHistory(store, context, from, now)
