...
fun PhysioTuningSection(modifier: Modifier = Modifier, onShowOnChart: (Long) -> Unit = {}) {
    ...
    val liveInsulin = remember {
        com.example.diapilot.data.PhysioRuntime
            .artifact(com.example.diapilot.data.Stores.get(context))
            ?.personModelAt(12.0, emptySet())?.insulin
    }
    val shownShape = remember {
        com.example.diapilot.data.ManualInsulinRuntime.shapeForCard(
            stored = com.example.diapilot.data.ManualInsulinRuntime.params(context),
    ...
    var isf by remember {
        mutableStateOf(s2(com.example.diapilot.data.ManualInsulinRuntime.params(context).isfMmolPerU))
    }
    ...
    fun commit() {
        ...
        com.example.diapilot.data.ManualInsulinRuntime.setParams(
            context,
            com.example.diapilot.data.ManualInsulinRuntime.shapeFromCard(
                onsetMin = num(on),
                peakMin = num(pk),
    ...
    Card(modifier = modifier.fillMaxWidth()) {
        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            val liveShape = com.example.diapilot.data.PhysioRuntime
                .artifact(com.example.diapilot.data.Stores.get(context))
                ?.personModelAt(12.0, emptySet())?.insulin
            Text(
            ...
            val artifactIsf by androidx.compose.runtime.produceState<
                ...
                value = withContext(Dispatchers.IO) {
                    runCatching {
                        ...
                        val a = com.example.diapilot.data.PhysioRuntime.artifact(
            ...
            val adaptive = remember(isf) {
                com.example.diapilot.data.AdaptiveIsfRuntime.state(context)
            }
            var source by remember {
                mutableStateOf(com.example.diapilot.data.IsfSource.choice(context))
            }
            Row(
                Modifier.fillMaxWidth(),
                horizontalArrangement = Arrangement.spacedBy(8.dp),
                ...
                androidx.compose.material3.Switch(
                    checked = source == com.example.diapilot.data.IsfSource.Choice.ADAPTIVE,
                    ...
                    onCheckedChange = { on ->
                        source = if (on) com.example.diapilot.data.IsfSource.Choice.ADAPTIVE
                        else com.example.diapilot.data.IsfSource.Choice.MANUAL
                        com.example.diapilot.data.IsfSource.set(context, source)
                    },
                )
            }
            Text(
                ...
                if (learned != null) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        TextButton(onClick = {
                            ...
                            isf = ""
                            val kept = com.example.diapilot.data.ManualInsulinRuntime.params(context)
                            com.example.diapilot.data.ManualInsulinRuntime.setParams(
